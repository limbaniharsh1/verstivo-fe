...
export function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <article className="product-card group relative cursor-pointer">
      ...
      {product.badge ? (
        <span
          className={`z-10 col-start-1 row-start-1 m-2 min-[375px]:m-2.5 sm:m-3 self-start justify-self-start rounded-full px-2 py-0.5 text-[9.5px] min-[375px]:text-[10px] sm:text-[10.5px] md:text-[11px] font-medium shadow-xs pointer-events-none ${badgeClassName}`}
        >
          {product.badge.label}
        </span>
      ) : null}
