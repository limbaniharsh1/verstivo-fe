...
export function ProductInfo({
  ...
  if (sizeParam) {
    const sizeIndex = product.sizes.findIndex(s => s.toLowerCase().includes(sizeParam.toLowerCase()));
    if (sizeIndex !== -1) {
      initialSize = `${product.sizes[sizeIndex]}-${sizeIndex}`;
    }
  }
  ...
  const getNumericSize = (sizeStr: string | null): number => {
    if (!sizeStr) return 0;
    const match = sizeStr.match(/\d+/);
    return match ? Number(match[0]) : 0;
  };
  const numericSize = getNumericSize(selectedSize);

  const variantKey = `${product.id}_${selectedColor}_${numericSize}`;
  const isVariantWishlisted = isLoggedIn
    ? (wishlistKeys || []).includes(variantKey)
    : false;
  ...
  return (
    <div className="flex flex-col w-full text-foreground">
      ...
      <div className="mt-5 sm:mt-6 xl:mt-7">
        <div className="flex flex-wrap items-center justify-between gap-1 text-xs sm:text-sm xl:text-base">
          <span className="font-semibold text-black">
            Select Size <span className="font-normal">· UK</span>
          </span>
          <button
            type="button"
      ...
      <div className="mt-5 sm:mt-6 flex flex-wrap min-[360px]:flex-nowrap items-center gap-2 sm:gap-3">
        ...
        >
          <HeartIcon
            filled={isVariantWishlisted}
            className="size-4.5 sm:size-5 transition-colors text-black"
          />
        </button>
      </div>
