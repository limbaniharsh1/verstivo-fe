...
import { PRIMARY_NAVIGATION, NavigationItem } from "@/constants/navigation";
...
export function SiteHeaderClient({ attributes }: { attributes?: BackendAttributes }) {
  ...
  const dynamicCollections: NavigationItem[] = (attributes?.collections || [])
    ...
    .map((c: any) => ({
      label: c.name,
      href: `/${c.slug || c._id}`,
      badge: c.showNewBadge ? "NEW" : undefined,
    }));

  const navigationItems: readonly NavigationItem[] = attributes?.collections
    ? [...baseNavigation, ...dynamicCollections]
    : PRIMARY_NAVIGATION;
        ...
        <div className="mx-auto flex h-[70px] 3xl:h-[82px] w-full max-w-[1750px] min-w-0 items-center ps-3 sm:ps-4 md:ps-5 xl:ps-13 3xl:ps-14.5 pe-4.5 xl:grid site-header-grid">
          <div className="flex h-full min-w-0 items-center gap-1.5 sm:gap-2 border-r-0 xl:border-r xl:border-border">
            ...
            >
              <ul className="flex h-full items-center gap-[34px] 3xl:gap-[38px]">
                {navigationItems.map(({ badge, href, label }) => {
          ...
          <div className="flex h-full w-full min-w-0 items-center justify-end">
            ...
            <button
              ...
              className={`${iconButtonClassName} hidden xl:grid w-[80px] 3xl:w-[92px] items-center justify-center`}
              aria-label="Account"
            >
              <UserRound className="size-4 2xl:size-5 mx-auto" strokeWidth={1.5} />
            </button>
            ...
            <button
              ...
              aria-label="Wishlist"
            >
              <HeartIcon className="size-4.5 2xl:size-5 mx-auto" />
            </button>
            ...
            <div className="p-5 space-y-4 bg-white">
              ...
              <div className="pt-3 border-t border-border flex items-center justify-around">
                <button
                  ...
                  className="flex items-center gap-2 text-xs font-semibold py-2 px-4 rounded-full text-foreground hover:bg-surface-muted transition-colors cursor-pointer"
                >
